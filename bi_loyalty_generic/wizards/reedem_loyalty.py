...
class ReedemptionLoyalty(models.TransientModel):
    ...
    redeem_point=fields.Integer("Reedemption Loyalty Point")  
    customer=fields.Char(string="Customer Name:",readonly=True)
    loyalty_point=fields.Float(string="Your Loyalty Points (in point(s)):",readonly=True)
    loyalty_balance=fields.Float(string="Your Loyalty Balance:",readonly=True)
    ...
    def ok_redeem(self):
        ...
        if config:  
            for rule in config.redeem_ids:
                # if rule.min_amt <= partner_id.loyalty_points  and   partner_id.loyalty_points <= rule.max_amt :
                if rule.min_amt <= partner_id.loyalty_points:
                    redeem_value = rule.reward_amt
                    point_amt=int(self.redeem_point) * redeem_value
                    redeem = True
            if redeem == False:    
                raise ValidationError(_('You dont have enough loyalty point, check your redeem configuration'))  
            loyalty_balance = round(redeem_value * partner_id.loyalty_points ,2)            
...
class SaleInherit(models.Model):
    ...
    loyalty_point_credit=fields.Float(string="Your Loyalty Balance:",readonly=True)
        ...
        if config : 
            for rec in self:
                ...
                if rec.order_redeem_points > 0:
                    ...
                    # partner_id.update({'loyalty_points':partner_id.loyalty_points - rec.redeem_value})
