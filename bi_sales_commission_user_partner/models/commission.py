						...
						for o_line in self.order_line :
							...
							if co.commission_level_id and order_product.is_commission_product == True:
								...
								if commission_p_line :
									commission_p_line.write({'amount' : commission_p_line.amount + (comm_filter_p.percentage * o_line.price_total)/100})
								
								else:
									commission_line_obj.sudo().create({'sales_partner': co.user_partner_id.id, 'commission_date': datetime.now().date(), 'source': self.name, 'amount': (comm_filter_p.percentage * o_line.price_total)/100}) 
						...
						for o_line in self.invoice_line_ids:
								...
								if co.commission_level_id and inv_product.is_commission_product == True:
									...
									if commission_p_line :
										...
										commission_line_obj.create({'sales_partner': co.user_partner_id.id, 'commission_date': datetime.now().date(), 'source': self.name, 'amount': (comm_filter_p.percentage * o_line.price_total)/100}) #, 'commission_id': sheet.id
...
class CreateCommissionPayment(models.Model):
	...
	# 								else:
	# 									commission_line_obj.create({'sales_partner': co.user_partner_id.id, 'commission_date': datetime.now().date(), 'source': rec.name, 'amount': (comm_filter_p.percentage * o_line.price_total)/100}) #, 'commission_id': sheet.id
