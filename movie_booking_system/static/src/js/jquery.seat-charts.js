	...
	$.fn.seatCharts = function (setup) {
		...
		var fn       = this,
			...
			seat = (function(seatCharts, seatChartsSettings) {
				return function (setup) {
					...
					// add icon wheelchair
					if(fn.settings.data.classes == 'handicapped'){
						icon_wheelchair = '<i class="fa fa-wheelchair" style="font-size:medium;"></i>'
						fn.settings.$node.empty()
					}
					fn.settings.$node.append(
						icon_wheelchair
					)
		...
		settings.naming.rows = settings.naming.rows || (function(length) {
			...
			for (var i = 0; i <= length; i++) {
				var a = (i+10).toString(36)
				rows.push(a);
			}
			return rows.reverse();
		})(settings.map.length-1);
		...
		$.each(settings.map, function(row, characters) {
			...
			if (settings.naming.left) {
				$row.append(
					$('<div></div>')
						.addClass('seatCharts-cell seatCharts-space first-row')
		...
		settings.legend.items.length ? (function(legend) {
			...
			$.each(legend.items, function(index, item) {
				var element_seat = ''
				// if handicapped add logo wheelchair
				if(item[1] == 'handicapped' || item[1] == 'handicapped unavailable'){
					element_seat = $('<div></div>')
					...
					.addClass(['seatCharts-seat', 'seatCharts-cell', item[1]].concat(
						...
						'<i class="fa fa-wheelchair" style="font-size:medium;"></i>'
					)
				}
				else{
					element_seat = $('<div></div>')
