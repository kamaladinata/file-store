...
class SaleOrder(models.Model):
	...
	def action_refause(self):
		...
		self.refused_by_id = self.env.user.id
		...
		for order in self :
			if order.company_id.three_step_approval == True and order.amount_total > order.company_id.so_double_validation_amount :
				order.write({
					'state': 'approve',
					'sale_manager':True,
					'sale_department_manager_id': self.env.user.id,
					'sale_manager_approval_date':datetime.datetime.now()})
				template_id = order.company_id.sale_approval_email_temp_id
				if template_id.id :
					send = template_id.send_mail(order.id, force_send=True)
			else :
				self.change_action_confirm()
		return True
	...
	def action_second_approve(self):
		if self.amount_total > self.company_id.finance_validation_amount :
			self.write({
				'state':'third_approve',
				'sale_director':True,
				'sale_director_id': self.env.user.id,
