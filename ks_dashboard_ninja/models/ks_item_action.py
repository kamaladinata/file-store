...
class KsDashboardNinjaBoardItemAction(models.TransientModel):
    ...
    def action_item_move_copy_action(self):
        #new version
        for each in self:
            if each.ks_action == 'move':
                for item in each.ks_dashboard_item_ids:
                    item.ks_dashboard_ninja_board_id = each.ks_dashboard_ninja_id
            elif each.ks_action == 'duplicate':
                # Using sudo here to allow creating same item without any security error
                for dashboard_id in each.ks_dashboard_ninja_ids:
                    for item in each.ks_dashboard_item_ids:
