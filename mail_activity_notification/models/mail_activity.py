...
class mail_activity(models.Model):
    """
    Re-write to introduce cron for activities' daily reminder
    """
    ...
    def cron_notify(self):
        ...
        task_ids = self.env["mail.activity"].search([
        ...
        for user in user_ids:
            try:
                user_task_ids = task_ids.filtered(lambda acti: acti.user_id == user)
