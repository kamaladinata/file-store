...
class pos_order(models.Model):
	...
	def create_picking(self):
		...
		for order in self:
			...
			# rename reference on account.move
			for rec_move in moves:
				for acc_move in rec_move.account_move_ids:
					ref_name = "%s - %s" %(order.name, order.session_id.name)
					acc_move.write({'ref':ref_name,})
