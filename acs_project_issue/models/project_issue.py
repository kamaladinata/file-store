...
class ProjectIssue(models.Model):
    ...
    def create(self, vals):
        vals['number'] = self.env['ir.sequence'].next_by_code('project.issue') or '/'
    ...
    def _track_template(self, changes):
        res = super(ProjectIssue, self)._track_template(changes)
        test_issue = self[0]
    ...
    def _track_subtype(self, init_values):
        ...
        if 'kanban_state' in init_values and self.kanban_state == 'blocked':
            return self.env.ref('acs_project_issue.mt_issue_blocked')
        elif 'kanban_state' in init_values and self.kanban_state == 'done':
            return self.env.ref('acs_project_issue.mt_issue_ready')
        elif 'user_id' in init_values and self.user_id:  # assigned -> new
            return self.env.ref('acs_project_issue.mt_issue_new')
        elif 'stage_id' in init_values and self.stage_id and self.stage_id.sequence <= 1:  # start stage -> new
            return self.env.ref('acs_project_issue.mt_issue_new')
        elif 'stage_id' in init_values:
            return self.env.ref('acs_project_issue.mt_issue_stage')
    ...
    def _notification_recipients(self, message, groups):
        ...
        if not self.user_id:
            ...
            new_action_id = self.env.ref('acs_project_issue.project_issue_categ_act0').id
