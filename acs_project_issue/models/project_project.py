...
class Project(models.Model):
    ...
    use_issues = fields.Boolean(string="Manage Issues", default=True)
    issue_needaction_count = fields.Integer(compute="_issue_needaction_count", string="Issues Count")
    ...
    @api.onchange('use_issues')
    def _on_change_use_issues(self):
        if self.use_issues:
...
class ProjectTask(models.Model):
    ...
    def _compute_issue_count(self):
        for task in self:
            task.issue_count = self.env['project.issue'].search_count([('task_id', '=', task.id)])
